...
(function () {
    ...
    const activeState = {
        ...
        adsense: {
            publisherId:
                (config.adsense && config.adsense.publisherId) || "",
            slots:
                (config.adsense && config.adsense.slots) || {}
        },

        adsterra: {
            domain:
                (config.adsterra && config.adsterra.domain) || "",
            slots:
                (config.adsterra && config.adsterra.slots) || {},
            directUnits:
                (config.adsterra && config.adsterra.directUnits) || {},
            keys: []
        }
    };
    ...
    */
    ...
    function renderDummy(slot, position) {
        ...
        positionBadge.textContent =
            "ADVERTISEMENT • " + String(position).toUpperCase();
    ...
    */
    ...
    function renderAdSense(slot, position) {
        ...
        // Normalize publisher ID
        if (
            !pubId.startsWith("ca-pub-") &&
            pubId.startsWith("pub-")
        ) {
            pubId = "ca-" + pubId;
        } else if (
            !pubId.startsWith("ca-pub-") &&
...
<script
    ...
    src="//${scriptDomain}/${slotKey}/invoke.js">
<\/script>
    ...
    function renderAdsterraDirectUnit(
        slot,
        unit
    ) {
        slot.innerHTML = "";
        ...
        script.dataset.zenlytaAdsterra =
            unit.containerId;
    ...
    */
    ...
    function fetchAndParseAdsTxt() {
        return new Promise((resolve) => {
            ...
            // Explicit provider = skip ads.txt detection
            if (
                config.provider &&
            ...
            function fallbackToConfig() {
                ...
                if (
                    ...
                    !config.adsense.publisherId.includes(
                        "XXXX"
                    )
                ) {
                    activeState.provider =
